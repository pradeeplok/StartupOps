...
const Validation = () => {
    ...
    // 1. Sentiment Score (0-100) * 0.4
    // 2. Feature Interest (Avg 0-100) * 0.4
    // 3. Data Volume (0-20) * 0.2 (Max 20 pts for 10+ feedbacks)
    const confidenceScore = React.useMemo(() => {
        const sentimentPart = sentimentMetrics.score * 0.4;

        const avgInterest = feedbackData.reduce((acc, curr) => acc + curr.value, 0) / (feedbackData.length || 1);
        const interestPart = avgInterest * 0.4;

        const volumeScore = Math.min(recentFeedback.length * 2, 20); // 2 pts per feedback, max 20 (10 feedbacks)
        const volumePart = volumeScore; // volumeScore is already out of 20, but we want it to be 20% of total. 20 * 1 = 20.

        return Math.round(sentimentPart + interestPart + volumePart);
    }, [sentimentMetrics.score, feedbackData, recentFeedback.length]);

    const getConfidenceColor = (score) => {
        if (score >= 75) return { bg: 'var(--bg-green-50)', text: 'var(--text-green-700)', border: 'var(--border-green-200)' };
        if (score >= 50) return { bg: 'var(--bg-blue-50)', text: 'var(--text-blue-700)', border: 'var(--border-blue-200)' };
        return { bg: 'var(--bg-orange-50)', text: 'var(--text-orange-700)', border: 'var(--border-orange-200)' };
    };
    ...
    return (
        <div style={{ maxWidth: '1152px', margin: '0 auto' }}>
            <div className="page-header flex">
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: confColor.bg, color: confColor.text, padding: '0.5rem 1rem', borderRadius: '9999px', border: `1px solid ${confColor.border}`, transition: 'all 0.3s ease' }}>
                    <Target size={20} />
                    <span style={{ fontWeight: 700 }}>Confidence Score: {confidenceScore}/100</span>
                </div>
            </div>
